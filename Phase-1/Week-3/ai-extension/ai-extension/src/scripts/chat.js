...
class ChatUI {
    ...
    getPromptKeys(language, engine) {
        ...
        // Playwright + TypeScript support path
        if (this.isPlaywrightTypeScript(lang, eng)) {
            if (isFeatureChecked && isPageChecked) {
                promptKeys.push('PLAYWRIGHT_TS_FEATURE_AND_STEPS');
        ...
        // Playwright + JavaScript support path
        if (this.isPlaywrightJavaScript(lang, eng)) {
            if (isFeatureChecked && isPageChecked) {
                promptKeys.push('PLAYWRIGHT_JS_FEATURE_AND_STEPS');
            } else if (isFeatureChecked) {
                promptKeys.push('PLAYWRIGHT_JS_FEATURE_ONLY');
            } else if (isPageChecked) {
     ...
     */
    isPlaywrightJavaScript(language, engine) {
    ...
    addUnsupportedLanguageMessage(language, engine) {
        const message = `⚠️ ${language}/${engine} combination is not yet supported. Currently supported combinations include Java/Selenium and TypeScript/Playwright.`;
